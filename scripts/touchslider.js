...
touchslider = {
    ...
    touchEnd: function(/*JQuery*/ elem, /*event*/ e) {
         ...
         }else if (touchslider.getLeft(elem) + $('.container').width() < $(window).width()) {
         //if (touchslider.getLeft(elem) + elem.parent().width() < $(window).width()) {
            //alert('touchslider.getLeft(elem) is '+touchslider.getLeft(elem));
            // alert('elem.parent().width() is '+elem.parent().width());
            // alert('container.width() is '+$('#thumbScroller').width());
            //  alert('$(window).width() is '+$(window).width());
             
            
             /*
              * This means they dragged to the left past the last item
              */
              touchslider.doSlide(elem,  '-'+($('.container').width()-$(window).width()), '1s');
...
$(document).ready(function() {
    touchslider.createSlidePanel('.container', 131, 2);
});
